...
def sanitize_file_base(stem: str) -> str:
    tokens = [t for t in re.split(r"[_\-\s]+", stem) if t]
    filtered = [t for t in tokens if t.lower() not in {"fake", "prank"}]
    cleaned = "_".join(filtered).strip("_")
    return cleaned or "off5_payload"
...
def maybe_write_xlsx(run_dir: Path, detail_rows: list[dict[str, Any]], summary_rows: list[dict[str, Any]], output_root: Path) -> Path | None:
    ...
    run_xlsx = run_dir / "reports" / "off5_xapk_ladder.xlsx"
    run_xlsx.parent.mkdir(parents=True, exist_ok=True)

    wb = Workbook()
    ws_d = wb.active
    ws_d.title = "OFF5_Ladder_Detail"
    if detail_rows:
    ...
    ws_s = wb.create_sheet("OFF5_Ladder_Summary")
    if summary_rows:
    ...
    if "OFF5_LADDER" not in mwb.sheetnames:
        mwb.create_sheet("OFF5_LADDER")
    ws = mwb["OFF5_LADDER"]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="OFF5 ladder: rung 1..N pushes unique filenames of same XAPK.")
    ...
    ap.add_argument("--device-dir", default="/sdcard/Download/bytebite_off5_ladder")
    ap.add_argument("--file-name-base", default="", help="Optional output filename base; fake/prank tokens are removed.")
    ap.add_argument("--between-s", type=float, default=0.5)
    ap.add_argument("--output-root", default="test_results/offensive_tests")
    ap.add_argument("--case-id", default="OFF5-XAPK-LADDER")
    args = ap.parse_args()
    ...
    print(f"[ByteBite] OFF5 ladder run: {run_dir}")
    ...
    chosen_base = args.file_name_base.strip() or src.stem
    base_stem = sanitize_file_base(chosen_base)
        ...
        summary_rows.append(
            {
                ...
                "device_dir_prepare_ms": round(mkdir_ms, 3),
                "total_time_ms": round(total_ms, 3),
                "total_time_including_dir_ms": round(total_ms + mkdir_ms, 3),
    ...
    detail_csv = reports / "off5_xapk_ladder_details.csv"
    summary_csv = reports / "off5_xapk_ladder_summary.csv"
